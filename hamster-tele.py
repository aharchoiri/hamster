...
TELEGRAM_BOT_TOKEN = '6799878702:AAE-PyrEr8tFAqyOsqx9CUBGOtErajIwh8M'
TELEGRAM_CHAT_ID = '1219820221'
...
def kirim_pesan_telegram(pesan):
    """Mengirim pesan ke Telegram dengan format yang menarik."""
    try:
        # Format pesan Telegram
        pesan_telegram = f"""
🐹  HamsterKombat Notification 🐹

👤 Username : @{pesan['username']}
ℹ️ Level : {pesan['level']}
...
Tips: Don't forget to upgrade mining!
"""
        bot.send_message(TELEGRAM_CHAT_ID, pesan_telegram)
        print(Fore.GREEN + Style.BRIGHT + f"\r[ Telegram ] : Pesan berhasil dikirim", flush=True)
    except Exception as e:
        print(Fore.RED + Style.BRIGHT + f"\r[ Telegram ] : Gagal mengirim pesan: {e}", flush=True)
    ...
    while True:
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 200:
                ...
                # Kumpulkan informasi untuk dikirim ke Telegram dalam dictionary
                pesan_telegram = {
                    'username': username,
                    'level': clicker_data['level'],
                    'balanceCoins': int(clicker_data['balanceCoins']),
                    'availableTaps': clicker_data['availableTaps'],
                    'exchangeId': clicker_data['exchangeId'],
                    'earnPassivePerHour': clicker_data['earnPassivePerHour']
                }
                
                # Kirim pesan ke Telegram
                kirim_pesan_telegram(pesan_telegram)
